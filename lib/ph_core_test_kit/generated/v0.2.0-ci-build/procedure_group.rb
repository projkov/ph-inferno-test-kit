...
require_relative 'procedure/procedure_id_search_test'
require_relative 'procedure/procedure_status_search_test'
require_relative 'procedure/procedure_encounter_search_test'
...
module PHCoreTestKit
  module PHCoreV020_CI_BUILD
    class ProcedureGroup < Inferno::TestGroup
      ...
      short_description 'Verify support for the server capabilities required by the PH Core Procedure.'
      description %(
  # Background

The PH Core Procedure sequence verifies that the system under test is
able to provide correct responses for Procedure queries. These queries
must contain resources conforming to the PH Core Procedure as
...
the [PH Core Procedure](https://fhir.doh.gov.ph/phcore/StructureDefinition/ph-core-procedure). Each element is checked against
...
read succeeds.
      ...
      id :ph_core_v020_ci_build_procedure
      run_as_group

      def self.metadata
        @metadata ||= InfernoSuiteGenerator::Generator::GroupMetadata.new(YAML.load_file(
                                                                            File.join(__dir__, 'procedure',
                                                                                      'metadata.yml'), aliases: true
                                                                          ))
      end

      test from: :ph_core_v020_ci_build_procedure_patient_search_test
      test from: :ph_core_v020_ci_build_procedure__id_search_test
      test from: :ph_core_v020_ci_build_procedure_status_search_test
      test from: :ph_core_v020_ci_build_procedure_encounter_search_test
      test from: :ph_core_v020_ci_build_procedure_code_search_test
      test from: :ph_core_v020_ci_build_procedure_date_search_test
      test from: :ph_core_v020_ci_build_procedure_read_test
      test from: :ph_core_v020_ci_build_procedure_validation_test
      test from: :ph_core_v020_ci_build_procedure_must_support_test
      test from: :ph_core_v020_ci_build_procedure_reference_resolution_test
    end
  end
end
