...
module PHCoreTestKit
  module PHCoreV020_CI_BUILD
    class OrganizationGroup < Inferno::TestGroup
      ...
      short_description 'Verify support for the server capabilities required by the PH Core Organization.'
      description %(
  # Background

The PH Core Organization sequence verifies that the system under test is
able to provide correct responses for Organization queries. These queries
must contain resources conforming to the PH Core Organization as
...
the [PH Core Organization](https://fhir.doh.gov.ph/phcore/StructureDefinition/ph-core-organization). Each element is checked against
...
read succeeds.
      ...
      id :ph_core_v020_ci_build_organization
      run_as_group

      def self.metadata
        @metadata ||= InfernoSuiteGenerator::Generator::GroupMetadata.new(YAML.load_file(
                                                                            File.join(__dir__, 'organization',
                                                                                      'metadata.yml'), aliases: true
                                                                          ))
      end

      test from: :ph_core_v020_ci_build_organization_read_test
