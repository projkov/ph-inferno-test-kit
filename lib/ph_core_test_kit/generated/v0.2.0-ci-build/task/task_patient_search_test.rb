...
module PHCoreTestKit
  module PHCoreV020_CI_BUILD
    class TaskPatientSearchTest < Inferno::Test
      include InfernoSuiteGenerator::SearchTest

      title '(SHALL) Server returns valid results for Task search by patient'
      description %(
A server SHALL support searching by
patient on the Task resource. This test
will pass if resources are returned and match the search criteria. If
none are returned, the test is skipped.

This test verifies that the server supports searching by reference using
the form `patient=[id]` as well as `patient=Patient/[id]`. The two
different forms are expected to return the same number of results. PH Core requires that both forms are supported by PH Core responders.
...
[PH Core Server CapabilityStatement](https://fhir.doh.gov.ph/phcore/CapabilityStatement/ph-core-server)
      ...
      id :ph_core_v020_ci_build_task_patient_search_test
      input :patient_ids,
            title: 'Patient IDs',
