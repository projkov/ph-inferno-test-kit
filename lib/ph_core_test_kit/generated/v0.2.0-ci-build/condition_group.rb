...
require_relative 'condition/condition_id_search_test'
require_relative 'condition/condition_encounter_search_test'
...
module PHCoreTestKit
  module PHCoreV020_CI_BUILD
    class ConditionGroup < Inferno::TestGroup
      ...
      short_description 'Verify support for the server capabilities required by the PH Core Condition.'
      description %(
  # Background

The PH Core Condition sequence verifies that the system under test is
able to provide correct responses for Condition queries. These queries
must contain resources conforming to the PH Core Condition as
...
the [PH Core Condition](https://fhir.doh.gov.ph/phcore/StructureDefinition/ph-core-condition). Each element is checked against
...
read succeeds.
      ...
      id :ph_core_v020_ci_build_condition
      run_as_group

      def self.metadata
        @metadata ||= InfernoSuiteGenerator::Generator::GroupMetadata.new(YAML.load_file(
                                                                            File.join(__dir__, 'condition',
                                                                                      'metadata.yml'), aliases: true
                                                                          ))
      end

      test from: :ph_core_v020_ci_build_condition_patient_search_test
      test from: :ph_core_v020_ci_build_condition__id_search_test
      test from: :ph_core_v020_ci_build_condition_encounter_search_test
      test from: :ph_core_v020_ci_build_condition_code_search_test
      test from: :ph_core_v020_ci_build_condition_clinical_status_search_test
      test from: :ph_core_v020_ci_build_condition_read_test
      test from: :ph_core_v020_ci_build_condition_validation_test
      test from: :ph_core_v020_ci_build_condition_must_support_test
      test from: :ph_core_v020_ci_build_condition_reference_resolution_test
    end
  end
end
