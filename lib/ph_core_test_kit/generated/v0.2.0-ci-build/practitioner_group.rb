...
require_relative 'practitioner/practitioner_active_search_test'
...
module PHCoreTestKit
  module PHCoreV020_CI_BUILD
    class PractitionerGroup < Inferno::TestGroup
      ...
      short_description 'Verify support for the server capabilities required by the PH Core Practitioner.'
      description %(
  # Background

The PH Core Practitioner sequence verifies that the system under test is
able to provide correct responses for Practitioner queries. These queries
must contain resources conforming to the PH Core Practitioner as
...
the [PH Core Practitioner](https://fhir.doh.gov.ph/phcore/StructureDefinition/ph-core-practitioner). Each element is checked against
...
read succeeds.
      ...
      id :ph_core_v020_ci_build_practitioner
      run_as_group

      def self.metadata
        @metadata ||= InfernoSuiteGenerator::Generator::GroupMetadata.new(YAML.load_file(
                                                                            File.join(__dir__, 'practitioner',
                                                                                      'metadata.yml'), aliases: true
                                                                          ))
      end

      test from: :ph_core_v020_ci_build_practitioner_read_test
      test from: :ph_core_v020_ci_build_practitioner__id_search_test
      test from: :ph_core_v020_ci_build_practitioner_identifier_search_test
      test from: :ph_core_v020_ci_build_practitioner_name_search_test
      test from: :ph_core_v020_ci_build_practitioner_active_search_test
      test from: :ph_core_v020_ci_build_practitioner_validation_test
      test from: :ph_core_v020_ci_build_practitioner_must_support_test
    end
  end
end
