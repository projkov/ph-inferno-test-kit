...
require_relative 'service_request/service_request_encounter_search_test'
...
require_relative 'service_request/service_request_reference_resolution_test'

module PHCoreTestKit
  module PHCoreV020_CI_BUILD
    class ServiceRequestGroup < Inferno::TestGroup
      title 'ServiceRequest Tests'
      short_description 'Verify support for the server capabilities required by the PH Core ServiceRequest.'
      description %(
  # Background

The PH Core ServiceRequest sequence verifies that the system under test is
able to provide correct responses for ServiceRequest queries. These queries
must contain resources conforming to the PH Core ServiceRequest as
...
the [PH Core ServiceRequest](https://fhir.doh.gov.ph/phcore/StructureDefinition/ph-core-serviceRequest). Each element is checked against
...
read succeeds.
      ...
      id :ph_core_v020_ci_build_service_request
      run_as_group

      def self.metadata
        @metadata ||= InfernoSuiteGenerator::Generator::GroupMetadata.new(YAML.load_file(
                                                                            File.join(__dir__, 'service_request',
                                                                                      'metadata.yml'), aliases: true
                                                                          ))
      end

      test from: :ph_core_v020_ci_build_service_request_patient_search_test
      test from: :ph_core_v020_ci_build_service_request__id_search_test
      test from: :ph_core_v020_ci_build_service_request_status_search_test
      test from: :ph_core_v020_ci_build_service_request_encounter_search_test
      test from: :ph_core_v020_ci_build_service_request_code_search_test
      test from: :ph_core_v020_ci_build_service_request_category_search_test
      test from: :ph_core_v020_ci_build_service_request_read_test
      test from: :ph_core_v020_ci_build_service_request_validation_test
      test from: :ph_core_v020_ci_build_service_request_must_support_test
      test from: :ph_core_v020_ci_build_service_request_reference_resolution_test
    end
  end
end
